...
(defun add (a b) (Pure (-> (Int Int) Int))
    (+ a b))
\end{lstlisting}
$x$ is a symbol denoting variable.
For example, $x \in \{a, b\}$ in Listing~\ref{src:vars}.
$\mathcal{T}$ is a symbol denoting type.
...
(data (List a)
    ...
    EList)
\end{lstlisting}
$t$ is a symbol denoting type variable.
For example, $t \in \{a\}$ in Listing~\ref{src:cons}.
$L$ is a symbol denoting label for user defined type.
...
$D$ is a symbol denoting user defined data.
...
\begin{figure}[tb]
    ...
    \begin{tabular}{rlrl}
        ...
        $\dfrac{S(L_{type}(L)) \equiv_\alpha \mathcal{T}}{\Gamma \vdash L : \mathcal{T}\ |_{FV_\mathcal{T}(\mathcal{T})}\ \varnothing}$ & (P-Label0) \vspace{5mm} \\
