...
impl<'t> Context<'t> {
    ...
    fn typing_defun(&self, mut defun: Defun<'t>) -> Result<Defun<'t>, TypingErr> {
        ...
        match unify(&fun_type1, &fun_type2) {
            None => {
                let msg = format!("function type was inferred as {:?} but defined as {:?}", fun_type2, fun_type1);
...
fn expr2data_expr(expr: &parser::Expr) -> Result<LangExpr, TypingErr> {
    ...
    match expr {
        ...
        _ => {
            return Err(TypingErr::new("data expression", expr));
        }
    }
...
fn expr2if(expr: &parser::Expr) -> Result<LangExpr, TypingErr> {
    ...
    match expr {
        ...
        _ => {
            return Err(TypingErr::new("if expression", expr));
        }
    }
